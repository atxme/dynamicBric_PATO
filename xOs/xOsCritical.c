...
int osCriticalDestroy(t_osCriticalCtx* p_pttOSCritical)
{
    ...
    // V�rifier que le compteur de verrou est � z�ro
    if (atomic_load(&p_pttOSCritical->a_usLockCounter) != 0)
    {
    ...
    // V�rifier que le flag du verrou indique bien que le mutex n'est pas verrouill�
    if (atomic_load(&p_pttOSCritical->a_bLock))
    {
        return OS_CRITICAL_ERROR;
    }

    // � ce stade, on consid�re que le mutex n'est plus en cours d'utilisation
